...
class Denoising_ksx4:
    ...
    def create_model(self, shape, stddev=0.1, skip_shortcuts=False, deconvolutional=False, loss='mse', padding='same'):
        ...
        if deconvolutional==False:
            x9 = Conv2D(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x8)
            x10 = Conv2D(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x9)
            x10 = BatchNormalization()(x10)
            x10 = Dropout(0.2)(x10)

            x11 = Conv2D(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x10)
            x12 = Conv2D(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x11)
            ...
            x9 = Conv2DTranspose(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x8)
            x10 = Conv2DTranspose(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x9)
            x10 = BatchNormalization()(x10)
            x10 = Dropout(0.2)(x10)
            ...
            x11 = Conv2DTranspose(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x10)
            x12 = Conv2DTranspose(self.kernel_size, self.filter_size, padding=padding, activation='relu')(x11)
            x12 = BatchNormalization()(x12)
            x12 = Dropout(0.2)(x12)
